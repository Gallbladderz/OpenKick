...
class SearchRepository(private val apiService: KickApiService) {
    fun searchStreamer(query: String): Flow<Result<SearchResultData>> = flow {
        try {
            ...
            val channels =
                response.data?.channels?.filter { it.slug.isNotBlank() }?.map { it.toDomain() }
                    ?: emptyList()
            val streams = response.data?.livestreams?.filter {
                (it.slug.ifBlank {
                    it.channel?.slug ?: ""
                }).isNotBlank()
            }?.map { it.toDomain() } ?: emptyList()
            val categories =
                response.data?.categories?.filter { it.slug.isNotBlank() }?.map { it.toDomain() }
                    ?: emptyList()
