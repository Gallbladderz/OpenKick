...
class FollowingRepository(private val apiService: KickApiService) {
    ...
    suspend fun fetchChannelDetails(slug: String): Result<FollowedStreamerUi> = withContext(Dispatchers.IO) {
        try {
    ...
    suspend fun fetchCategoryDetails(slug: String): Result<FollowedCategoryUi> = withContext(Dispatchers.IO) {
        try {
...
fun ChannelV1Response.toFollowedStreamerDomain(slugFallback: String): FollowedStreamerUi {
    ...
    return if (this.livestream != null) {
        ...
        FollowedStreamerUi(slugFallback, username, avatarUrl, true, title, viewers, categoryName, streamThumbnailUrl)
    } else {
